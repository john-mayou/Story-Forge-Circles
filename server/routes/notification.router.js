...
router.get("/", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(notificationsQuery, [req.user.id])
    .then((result) => {
...
router.post("/add-member", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(newMemberInsertion, [user_id, circle_id])
    .then(() => {
      res.sendStatus(201);
    })
    .catch((error) => {
...
router.put("/new-leader", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(newLeaderUpdateQuery, [new_leader, circle_id])
    .then(() => {
      res.sendStatus(201);
    })
    .catch((error) => {
...
router.put("/complete/:id", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(completionUpdate, [req.params.id])
    .then(() => {
      res.sendStatus(201);
    })
    .catch((error) => {
